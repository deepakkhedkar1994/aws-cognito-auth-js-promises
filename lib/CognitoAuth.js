...
var CognitoAuth = function () {
  ...
  CognitoAuth.prototype.getCurrentUser = async function getCurrentUser() {
    var lastUserKey = 'CognitoIdentityServiceProvider.' + this.clientId + '.LastAuthUser';

    var lastAuthUser = await this.storage.getItem(lastUserKey);
    return lastAuthUser;
  };
  ...
  CognitoAuth.prototype.getSession = async function getSession() {
  ...
  CognitoAuth.prototype.parseCognitoWebResponse = async function parseCognitoWebResponse(httpRequestResponse) {
    var map = void 0;
    if (httpRequestResponse.indexOf(this.getCognitoConstants().QUESTIONMARK) > -1) {
      ...
      var _response2 = await this.getTokenQueryParameter(map);
      return _response2;
    }
  };
  ...
  CognitoAuth.prototype.getCodeQueryParameter = async function getCodeQueryParameter(map) {
    var state = null;
    ...
    if (map.has(this.getCognitoConstants().CODE)) {
      ...
      var _response = await this.makePOSTRequest(header, body, url);
  ...
  CognitoAuth.prototype.getTokenQueryParameter = async function getTokenQueryParameter(map) {
    var idToken = new _CognitoIdToken2.default();
    var accessToken = new _CognitoAccessToken2.default();
    var refreshToken = new _CognitoRefreshToken2.default();
    var state = null;
  ...
  CognitoAuth.prototype.getCachedSession = async function getCachedSession() {
    if (!this.username) {
    ...
    var scopesString = await this.storage.getItem(scopeKey);
    var scopesArray = [];
    if (scopesString) {
      scopesArray = scopesString.split(' ');
    }
    var tokenScopes = new _CognitoTokenScopes2.default(scopesArray);
    var idToken = new _CognitoIdToken2.default((await this.storage.getItem(idTokenKey)));
    var accessToken = new _CognitoAccessToken2.default((await this.storage.getItem(accessTokenKey)));
    var refreshToken = new _CognitoRefreshToken2.default((await this.storage.getItem(refreshTokenKey)));
  ...
  CognitoAuth.prototype.getLastUser = async function getLastUser() {
    var keyPrefix = 'CognitoIdentityServiceProvider.' + this.getClientId();
    var lastUserKey = keyPrefix + '.LastAuthUser';
    var lastUserName = await this.storage.getItem(lastUserKey);
    if (lastUserName) {
      return lastUserName;
    }
    return undefined;
  ...
  CognitoAuth.prototype.cacheTokensScopes = async function cacheTokensScopes() {
    var keyPrefix = 'CognitoIdentityServiceProvider.' + this.getClientId();
    ...
    await this.storage.setItem(idTokenKey, this.signInUserSession.getIdToken().getJwtToken());
    await this.storage.setItem(accessTokenKey, this.signInUserSession.getAccessToken().getJwtToken());
    await this.storage.setItem(refreshTokenKey, this.signInUserSession.getRefreshToken().getToken());
    await this.storage.setItem(lastUserKey, tokenUserName);
    await this.storage.setItem(scopeKey, scopesString);
  };
  ...
  CognitoAuth.prototype.clearCachedTokensScopes = async function clearCachedTokensScopes() {
    var keyPrefix = 'CognitoIdentityServiceProvider.' + this.getClientId();
    ...
    await this.storage.removeItem(idTokenKey);
    await this.storage.removeItem(accessTokenKey);
    await this.storage.removeItem(refreshTokenKey);
    await this.storage.removeItem(lastUserKey);
    await this.storage.removeItem(scopeKey);
  };
  ...
  CognitoAuth.prototype.refreshSession = async function refreshSession(refreshToken) {
    ...
    //const boundOnSuccess = (await this.onSuccessRefreshToken).bind(this);
    //const boundOnFailure = (this.onFailure).bind(this);
    //this.makePOSTRequest(header, body, url, boundOnSuccess, boundOnFailure);
    var response = await this.makePOSTRequest(header, body, url);
    var signInUserSession = await this.onSuccessRefreshToken(response);
  ...
  CognitoAuth.prototype.onSuccessRefreshToken = async function onSuccessRefreshToken(jsonData) {
  ...
  CognitoAuth.prototype.onSuccessExchangeForToken = async function onSuccessExchangeForToken(jsonData) {
    var jsonDataObject = JSON.parse(jsonData);
    var refreshToken = new _CognitoRefreshToken2.default();
    var accessToken = new _CognitoAccessToken2.default();
    var idToken = new _CognitoIdToken2.default();
  ...
  CognitoAuth.prototype.signOut = async function signOut() {
    var URL = this.getFQDNSignOut();
    this.signInUserSession = new _CognitoAuthSession2.default();
    await this.clearCachedTokensScopes();
    this.launchUri(URL);
  };
  ...
  CognitoAuth.prototype.isUserSignedIn = async function isUserSignedIn() {
    var cachedSession = await this.getCachedSession();
    return this.signInUserSession != null && this.signInUserSession.isValid() || cachedSession != null && cachedSession.isValid();
  };
